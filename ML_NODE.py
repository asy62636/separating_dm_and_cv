...
class opencv_algo():
    ...
    def run_algo(self):
        ...
        bridge = CvBridge()
        warped1 = bridge.cv2_to_imgmsg(warped,"bgr8")
        warped_pub = rospy.Publisher('/cv/warpedimg', Image, queue_size=10)
        warped_pub.publish(warped1)
        ...
        x0_arr = Int32MultiArray()
        x0_arr.data = x0
        x0_pub = rospy.Publisher('/cv/x0', Int32MultiArray, queue_size=10)
        x0_pub.publish(x0_arr)

        x1_arr = Int32MultiArray()
        x1_arr.data = x1
        x1_pub = rospy.Publisher('/cv/x1', Int32MultiArray, queue_size=10)
        x1_pub.publish(x1_arr)
        
        x2_arr = Int32MultiArray()
        x2_arr.data = x2
        x2_pub = rospy.Publisher('/cv/x2', Int32MultiArray, queue_size=10)
        x2_pub.publish(x2_arr)
        
        y0_arr = Int32MultiArray()
        
        y0_arr.data = y0
        y0_pub = rospy.Publisher('/cv/y0', Int32MultiArray, queue_size=10)
        y0_pub.publish(y0_arr)
        
        y1_arr = Int32MultiArray()
        y1_arr.data = y1
        y1_pub = rospy.Publisher('/cv/y1', Int32MultiArray, queue_size=10)
        y1_pub.publish(y1_arr)
        
        y2_arr = Int32MultiArray()
        y2_arr.data = y2
        y2_pub = rospy.Publisher('/cv/y2', Int32MultiArray, queue_size=10)
        y2_pub.publish(y2_arr)
        
        '''
        list_of_xy = [x0,y0,x1,y1,x2,y2]
        rate_for_xy = rospy.Rate(1)
        for i in list_of_xy:
            xy_publisher = rospy.Publisher('/cv/xylist', Int32MultiArray, queue_size=1)
            xy_publisher.publish(i)
            rate_for_xy.sleep()
        '''
